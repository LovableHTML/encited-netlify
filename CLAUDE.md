...
Flow: team surface stores the Encited API key in team configuration → site surface enable writes two env vars to the site (`ENCITED_API_KEY` secret, builds/functions/runtime scopes, **production context only** — Netlify 422s secrets on "all contexts", and production-scoping keeps the edge function out of preview/branch builds; plus `ENCITED_PRERENDER_ENABLED`) and triggers a redeploy → `shouldInjectFunction` in `src/index.ts` sees those env vars during the site's build and injects the edge function. Saving a new team key propagates the env var to every enabled site and redeploys them (`teamSettings.mutate`), so key rotation can't silently strand sites on a revoked key. The extension installs team-wide; this env-var gate is the only thing keeping the edge function out of every other site the team owns. Never weaken it.
